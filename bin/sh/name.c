...
struct namnod mailnod = { (struct namnod *)NIL, (struct namnod *)NIL,
	mailname };
struct namnod mchknod = { &ifsnod, &ps1nod, mchkname };
struct namnod acctnod = { (struct namnod *)NIL, (struct namnod *)NIL,
	acctname };
struct namnod mailpnod = { (struct namnod *)NIL, (struct namnod *)NIL,
	mailpname };
...
setlist(arg, xp) register struct argnod *arg;
int xp;
{
	if (flags & exportflg)
		xp |= N_EXPORT;
...
readvar(names) char **names;
{
	struct fileblk fb;
	register struct fileblk *f = &fb;
	register char c;
	register int rc = 0;
	struct namnod *n = lookup(
	    *names++); /* done now to avoid storage mess */
...
exname(n) register struct namnod *n;
{
...
printro(n) register struct namnod *n;
{
	if (n->namflg & N_RDONLY) {
		prs_buff(readonly);
		prc_buff(SP);
...
printexp(n) register struct namnod *n;
{
...
unset_name(name) register char *name;
{
	register struct namnod *n;
	...
	if (n = findnam(name)) {
		...
		if (n == &pathnod || n == &ifsnod || n == &ps1nod ||
		    n == &ps2nod || n == &mchknod) {
