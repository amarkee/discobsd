...
estabf(char *s)
{
	...
	(f = standin)->fdes = -1;
	f->fend = strlen(s) + (f->fnxt = s);
	f->flin = 1;
	return (f->feof = (s == NIL));
}

push(af) struct fileblk *af;
{
	register struct fileblk *f;
...
rename(f1, f2) register int f1, f2;
{
...
copy(ioparg) struct ionod *ioparg;
{
	register char *cline;
...
swap_iodoc_nm(i) struct ionod *i;
{
	while (i) {
		sh_free(i->ioname);
		i->ioname = i->iolink;
...
restore(last) register int last;
{
	register int i;
	register int dupfd;
