 ...
 */
initio(iop, save) struct ionod *iop;
int save;
{
	register char *ion;
	register int iof, fd;
...
pathopen(path, name) register char *path, *name;
{
	register int f;
...
gocsh(t, cp, xecenv) register char **t, *cp, **xecenv;
{
	char *newt[1000];
	register char **p;
	register int i;
...
int argn;
{
	register struct argnod *argp = (struct argnod *)(Rcheat(gchain) &
	    ~ARGMK);
